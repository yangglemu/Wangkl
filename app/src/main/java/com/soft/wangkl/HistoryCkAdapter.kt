...
/**
 * Created by yangglemu on 2016/7/16.
 */
...
class HistoryCkAdapter(activity: MainActivity, db: SQLiteDatabase, start: Date?, end: Date?) : DataAdapter(activity, db, start, end) {
    override fun initData() {
        ...
        var sql = "select date(rq) as rq,tm,sl from ck "
    ...
    override fun setSort(v: View) {
        val tm = v.findViewById(R.id.history_header_tm)
        val sl = v.findViewById(R.id.history_header_sl)
        val rq = v.findViewById(R.id.history_header_rq)
        val je = v.findViewById(R.id.history_header_je)
        setClick(tm, "tm")
        setClick(sl, "sl")
        setClick(rq, "rq")
        setClick(je, "je")
